...
void Constrained_LS::initialize(FullMatrix<double> A, FullMatrix<double> C) {
  ...
  gsl_vector_free(tau);
  gsl_vector_free(tau_bar);
  gsl_matrix_free(qr);
  gsl_matrix_free(qr_bar);
  gsl_matrix_free(q);
  gsl_matrix_free(q_bar);
  gsl_matrix_free(r);
  gsl_matrix_free(r_bar);
}
...
Constrained_LS::Constrained_LS(FullMatrix<double> A) { initialize(A); }
...
void Constrained_LS::solve(Vector<double> b, Vector<double> d,
                           Vector<double> &x) {
