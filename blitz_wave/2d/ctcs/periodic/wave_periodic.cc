...
void write_tecplot(int n,double t,const Array<double, 1> &x, const Array<double, 1> &y, const Array<double, 2> &u)
{
    ...
    int nx(x.numElements()), ny(y.numElements());
    tpl << "TITLE = \"Wave Equation 2D\" " << std::endl << "VARIABLES = \"x\", \"y\", \"u\" " << std::endl;
...
void wave(double tf = 1.0)
{
  ...
  const double kx(k), ky(k); //wave number x and y components:
  ...
  //plane wave solution 
  double t;
  auto plane_wave = [&t,kx,ky,omega](auto x,auto y){return sin(kx*x + ky*y - omega*t);};
  ...
  int nt = 0;
  t = 0;
  auto P1 = project(plane_wave,x,y);
  write_tecplot(nt,t,x,y,P1);

  //initial condition P2:
  nt++;
  t += dt;
  auto P2 = project(plane_wave,x,y);
